...
def ensure_demo_knowledge_articles(db: Session, volunteer_user_id: int, organization_user_id: int) -> None:
    rows: list[tuple[str, str, str, str, bool, int, str]] = [
        (
            ...
            "care",
            """Первые две недели в новом доме — самый уязвимый период для кошки. Питание в это время должно быть предсказуемым: одинаковые часы, одна и та же миска, минимум резких изменений.

Если вы знаете, чем кормили кошку в приюте или у прежних хозяев, сохраните этот корм минимум на 7–10 дней. Переход на новый рацион делайте постепенно: в первый день 75% старого корма и 25% нового, к пятому дню — 50/50, к десятому можно перейти полностью.

Для взрослой кошки среднего размера ориентир — 50–70 г сухого корма в сутки или 200–250 г влажного, но точная норма зависит от активности, возраста и состояния. Лучше чаще давать небольшие порции, чем одну большую: 3–4 кормления в день снижают риск переедания и стресса.

Обязательно должен быть постоянный доступ к свежей воде. Миски ставьте подальше от лотка и в тихом месте, куда кошка уже ходит без страха.

Не давайте с ладони «со стола»: солёное, жирное и сладкое опасно. Если кошка отказывается есть больше суток, но пьёт — это повод связаться с куратором или ветеринаром, а не менять корм каждый день.

Признаки, что рацион подходит: стабильный аппетит, нормальный стул, кошка не теряет в весе и постепенно осваивает дом. Если появляется рвота, понос или резкая вялость — нужна консультация специалиста.""",
            False,
        ...
        (
            ...
            "first_aid",
            """Небольшой порез на подушечке или между пальцами у собаки и кошки встречается часто: стекло, лед, острый предмет на прогулке. Главное — не паниковать и оценить глубину раны.

Остановите кровотечение: прижмите чистую марлю или бинт на 5–10 минут. Если кровь сильная и не останавливается — это уже повод ехать в клинику сразу, а не ждать.

Когда кровотечение остановилось, промойте рану физраствором или чистой водой без мыла и антисептиков с спиртом — они обжигают ткань. Вокруг можно аккуратно обработать хлоргексидином, но не заливайте им саму глубокую рану.

Если порез поверхностный, без видимых сухожилий и кости, накройте стерильной повязкой и ограничьте активность: без длинных прогулок, без бега по лестницам. Конус или защитный воротник помогут, если животное постоянно разлизывает место.

Срочно к ветеринару, если: рана глубокая, кровь пульсирует, лапа опухла и горячая, животное не наступает на конечность, есть инородное тело в ране или признаки инфекции через 24–48 часов (гной, резкий запах, покраснение по краям).

Не давайте человеческие обезболивающие без назначения врача — многие из них токсичны для кошек и собак. До осмотра достаточно покоя, повязки и наблюдения.""",
            True,
            volunteer_user_id,
            "volunteer",
        ),
        (
            ...
            "Режим, кормление и наблюдение за состоянием после визита к ветеринару.",
            "treatment",
            """После стационара или амбulatorного лечения животному нужен спокойный режим. Ветеринар обычно выдаёт памятку — следуйте ей буквально: дозировки, время приёма препаратов и ограничения по активности не «примерные», а критичные для выздоровления.

Первые сутки дома держите питомца в одной тихой комнате с лежанкой, водой и, при необходимости, лотком или пелёнками. Дети и другие животные — только под вашим контролем, без самостоятельных «обнимашек».

Кормление после наркоза или операции часто сдвигают: сначала вода, через несколько часов — лёгкий корм, который рекомендовал врач. Не угощайте привычным лакомством «для настроения», если это не согласовано.

Ежедневно осматривайте швы, повязки и место укола: покраснение, отёк, выделения — повод позвонить в клинику. Фото швов в динамике помогают врачу дистанционно оценить процесс.

Таблетки давайте строго по времени. Если животное выплёвывает препарат — не удваивайте дозу без консультации. Для капель и мазей вымойте руки и фиксируйте питомца спокойно, без силы.

Признаки ухудшения, при которых нужна срочная помощь: отказ от воды более полусуток, повторная рвота, затруднённое дыхание, кровотечение из шва, температура, выраженная вялость.

Запишите дату следующего осмотра и не пропускайте контроль — даже если снаружи всё зажило. Внутреннее восстановление часто длится дольше, чем кажется по поведению.

После кастрации и стерилизации ограничьте прыжки и активные игры 10–14 дней. Конус или защитный воротник носят по назначению врача — снятие «потому что жалко» часто приводит к расхождению шва.

У кошек после операции следите за аппетитом: отказ от еды более суток при нормальном питье — повод позвонить в клинику. У собак после ортопедических вмешательств режим строже: короткие выходы только на поводке, без лестниц и диванов.

Обезболивающие и антибиотики — только по схеме. Пропущенная доза не компенсируется удвоением без звонка врачу. Храните препараты в недоступном для животного месте, фиксируйте время приёма в заметках или будильнике.

Повязки и воротники меняют по инструкции. Мокрая или грязная повязка — риск инфекции. Если животное снимает защиту — не оставляйте без присмотра, используйте альтернативу, которую рекомендовал врач.

Домашняя реабилитация после травмы включает физиотерапию по назначению: пассивные движения, короткие прогулки, постепенное увеличение нагрузки. Резкое «пустим побегать, скучно» откатывает прогресс на недели.

Стресс замедляет выздоровление. Тихая комната, привычный запах лежанки, спокойный голос важнее «развлечений гостями». Дети должны понимать: сейчас не время для активных игр.

Фото и видео для врача снимайте при дневном свете: общий вид, крупно шов или место укола, если есть выделения — отдельный кадр. Так экономите время на повторном визите.

Если назначена диета — не подменяйте «чем-то вкуснее». ЖКТ после наркоза и антибиотиков чувствителен; возврат к обычному корму только когда разрешит специалист.

Держите под рукой телефон клиники и номер доверенного ветеринара. В нерабочие часы знайте, куда обращаться в экстренном порядке — это часть ответственного ухода после лечения.""",
            True,
            organization_user_id,
            "organization",
        ),
        (
            ...
            "Кормление, гигиена и безопасное обращение с подопечной.",
            "care",
            """Работа с кошкой в приюте начинается с наблюдения. Не лезьте сразу гладить: дайте время понюхать руку, говорите спокойным тоном, избегайте резких движений над головой.

Кормление по расписанию приюта — часть адаптации. Миски моют после каждого кормления, вода меняется минимум раз в день. Если кошка стесняется есть при вас — отойдите на пару метров.

Лоток: убирайте комки ежедневно, полная замена наполнителя — по регламенту вольера. Одна кошка — минимум один лоток; в временной передержке лучше два в разных уголках.

Гигиена шерсти: короткошёрстным достаточно расчёсывания 2–3 раза в неделю, длинношёрстным — чаще. Проверяйте уши, глаза, подушечки лап без давления. Любое выделение из глаз или ушей — отметка куратору.

Игрушки и когтеточка снижают стресс. Не оставляйте в клетке предметы, которые можно проглотить: нитки, мелкие детали, пластик.

При укусе или царапине — не наказывайте: отойдите, дайте кошке выйти из стресса. Запишите инцидент для смены: возможно, животному нужен более тихий вольер или дополнительная социализация.

Безопасность волонтёра: перчатки при уборке, отдельные инструменты для больных животных, мытьё рук между вольерами. Это защищает и людей, и подопечных.""",
            True,
            organization_user_id,
            "organization",
        ),
        (
            ...
            "legal",
            """Пристройство — это не «отдать в хорошие руки на словах», а понятная процедура с документами. Она защищает животное, новых хозяев и саму организацию.

Базовый пакет обычно включает: договор ответственного содержания (или договор дарения), акт приёма-передачи, паспорт животного при наличии, выписку о вакцинации и обработке от паразитов, рекомендации по уходу и корму.

В договоре фиксируют: данные сторон, описание животного (кличка, вид, окрас, приметы), обязательство не бросать, не передавать третьим лицам без согласия организации, условия возврата, если семья не справляется.

Важно прописать, что организация вправе уточнять условия содержания — не как слежка, а как механизм помощи. Добровольные фотоотчёты и короткие созвоны снижают риск возврата «с сюрпризом».

Несовершеннолетние не могут быть стороной договора без законного представителя. Если новый дом — семья с детьми, подпись даёт взрослый, который несёт ответственность.

Частные лица, передающие найденное животное, тоже могут оформить простой акт — это снижает споры о собственности позже.

Храните сканы договоров минимум три года. При споре или возврате животного документы — единственная опора.

Если усыновитель отказывается подписывать договор — это красный флаг. Лучше отложить передачу, чем рисковать благополучием подопечного.

Перед подписанием имеет смысл коротко проверить условия: живут ли в доме другие животные, есть ли дети, кто будет основным ответственным за уход. Это не «допрос», а снижение риска возврата через неделю.

Домашний визит или видеозвонок по согласованию помогает увидеть реальную обстановку: сетки на окнах для кошек, высота забора для собак, где будет спать животное. Фиксируйте договорённости письменно, даже если визит неформальный.

В акте приёма-передачи укажите известные медицинские особенности: хронические заболевания, аллергии, принимаемые препараты, дата последней вакцинации. «Здоров на глаз» — плохая формулировка, если есть выписка из клиники.

Обсудите сценарий возврата без стыда и конфликта: куда звонить, в какой срок, кто организует транспорт. Чем понятнее процедура, тем меньше животных оказываются «просто на улице».

Частные объявления «отдам в добрые руки» без документов создают серую зону. Организация может предложить помощь с оформлением — это повышает доверие к приюту и защищает переданное животное.

При передаче щенка или котёнка приложите памятку по социализации, графику кормления и контактам куратора на первые две недели. Новым хозяевам проще не ошибиться, когда всё собрано в одном месте.

Если животное чипировано — проверьте, что данные в базе обновлены на нового владельца или что процедура перерегистрации описана в договоре. Иначе при потере найти семью будет сложнее.

Храните сканы не только у себя: отправьте копию усыновителю на почту — это снижает споры о «мы ничего не подписывали». Электронная переписка с датой тоже часть доказательной базы.

Юридическая грамотность не заменяет человеческого отбора семьи, но закрывает типовые дыры: перепродажа, брошенное животное, отсутствие медицинской истории. Документы — каркас, на который опирается забота.""",
            False,
            organization_user_id,
            "organization",
        ),
        (
            "Как правильно кормить собаку после передержки",
            "Переход на домашний рацион, режим и типичные ошибки новых хозяев.",
            "care",
            """Собака после передержки или приюта привыкла к определённому корму и графику. Резкая смена рациона — частая причина поноса в первые дни дома.

Уточните у куратора, чем и сколько раз в день кормили последнюю неделю. Купите тот же корм на 1–2 недели вперёд. Если хотите перейти на другой — смешивайте старый и новый постепенно, как при адаптации кошки.

Взрослой собаке среднего размера часто подходит два кормления в день, щенку — три–четыре. Миска убирается через 15–20 минут, если собака не доела: так формируется дисциплина, а не «шведский стол».

Вода должна быть всегда. После активной прогулки не кормите сразу — подождите 30–40 минут, особенно у крупных пород.

Лакомства на прогулке — инструмент дрессировки, а не основная еда. Считайте их в суточную норму, иначе собака набирает вес, не доедая основной корм.

Типичные ошибки: слишком много «человеческой еды», молоко взрослой собаке, кости от стола, резкая смена корма «потому что дорогой лучше».

Если понос длится больше двух дней, есть кровь в стуле или рвота — ветеринар, а не новая марка корма из интернета.""",
            False,
            volunteer_user_id,
            "volunteer",
        ),
        (
            "Социализация пугливой собаки",
            "Пошаговый план привыкания к людям, звукам и прогулкам без перегруза.",
            "socialization",
            """Пугливая собака не «злая» и не «упрямая» — она напугана. Социализация строится на предсказуемости и маленьких победах, а не на «вытаскивании из зоны комфорта».

Начните дома: пусть собака сама подходит за лакомством, не тяните к себе. Сидите на полу, не наклоняйтесь сверху. Награждайте спокойное поведение, а не только прикосновения.

Звуки бытовой техники, дверной звонок, шаги на лестнице — включайте постепенно на низкой громкости или на записи, если собака паникует на улице. Параллельно — что-то вкусное и спокойная обстановка.

Первая прогулка — короткая, в тихое время, на знакомой территории у приюта или во дворе. Не тащите на оживлённую площадь «для закалки». Шлея и мягкая поводок лучше рывков и удушающих ошейников.

Если собака замирает — остановитесь, дайте отойти в безопасную дистанцию. Цель — чтобы она сама выбирала исследовать, а не терпела.

С другими собаками знакомьте только после стабилизации на прогулке один на один с человеком. Сначала — спокойные, взрослые, предсказуемые собаки на дистанции.

Ведите дневник: что получилось, что вызвало страх, на каком расстоянии. Так видно прогресс, который иначе кажется невидимым.

При агрессии из страха (рычание, попытка укуса) — не наказывайте. Обратитесь к кинологу с опытом работы с травмированными собаками. Иногда нужна связка с ветеринаром для оценки тревожности.""",
            False,
            volunteer_user_id,
            "volunteer",
        ),
        (
            ...
            "Первые действия, что нельзя делать дома и какая информация нужна ветеринару.",
            "first_aid",
            """Подозрение на отравление — ситуация, где счёт идёт на часы. Не ждите «может само пройдёт» и не лечите народными методями.

Симптомы могут быть разными: рвота, понос, слюнотечение, судороги, вялость, странное поведение, расширенные или суженные зрачки. Иногда признаки появляются не сразу — особенно при медленно действующих ядах.

Если вы видели, что животное съело — сохраните упаковку, остатки вещества, фото. Это ускорит помощь в клинике. Не вызывайте рвоту самостоятельно без указания врача: при кислотах, щелочах и некоторых химикатах это опасно.

Свяжитесь с ближайшей клиникой или службой экстренной помощи для животных и едьте туда, а не «сначала в интернет». По телефону сообщите вид, вес, время события, симптомы.

До приезда — тишина, тепло, не давайте еду и воду, если животное без сознания или сильно рвёт. В сознании иногда можно предложить воду маленькими глотками, если так сказал врач.

Не давайте молоко, масло, активированный уголь «как всегда у людей» без дозировки от ветеринара — для животных схема другая.

На профилактике: убирайте антифриз, грызуновод, лекарства, шоколад, виноград, лук — в недоступные места. На прогулке следите, чтобы собака не подбирала с земли.

После стабилизации врач может назначить диету и наблюдение дома. Строго соблюдайте режим — рецидив на фоне «почти выздоровела» случается часто.""",
            True,
            organization_user_id,
            "organization",
        ),
        (
            "Первая смена волонтёра в приюте",
            "Что взять с собой, как вести себя с животными и сменой, типичные ошибки новичков.",
            "care",
            """Первая смена в приюте волнует даже опытных людей: незнакомые правила, много животных, хочется успеть «всем помочь». Лучший результат — спокойное следование инструкции куратора, а не героизм в одиночку.

Приходите вовремя, в закрытой обуви и одежде, которую не жалко испачкать. Возьмите бутылку воды, перекус, телефон на беззвучном режиме. Крупные серьги, длинные шарфы и открытые раны на руках — лишний риск при работе с испугавшимися собаками и кошками.

Перед входом в вольеры вымойте руки и используйте сменную обувь или бахилы, если так принято в приюте. Запахи с улицы и других клиник могут пугать животных и переносить инфекции между зонами.

Куратор покажет маршрут: где корм, где аптечка, где мусор, как открывать двери, чтобы никто не выскочил. Запишите или сфотографируйте схему — в середине смены память подводит.

Не открывайте клетки без разрешения, даже если животное «милое и точно доброе». Стресс, боль, незнакомый человек — достаточные причины для укуса. Сначала представьтесь голосом, потом медленно откройте, не заглядывая сверху в лицо кошке.

Кормление и уборка — по регламенту, а не «как дома у себя». Другой корм, другой наполнитель или «лишняя» порция лакомств могут вызвать понос у чувствительного животного и сорвать план смены.

Фото и stories в соцсетях — только с разрешения приюта. Не публикуйте адрес, номера вольеров с ослабленными животными, лица других волонтёров без согласия. Это вопрос безопасности и репутации организации.

Если животное ведёт себя агрессивно или вяло — не наказывайте и не «дрессируйте силой». Отметьте куратору: возможно, нужен ветеринар или другой вольер. Ваша задача — безопасность, а не перевоспитание за два часа.

Перерывы нужны: полчаса без телефона в коридоре лучше, чем выгорание через три смены. Приют ценит стабильных волонтёров, которые приходят регулярно, а не один раз на износ.

В конце смены сдайте ключи, отметьте выполненные задачи в журнале или чате, сообщите о любых странностях: кто не ел, кто хромает, что сломалось. Следующая смена начнётся с ваших записей.

Типичные ошибки новичков: слишком много говорить и гладить сразу; кормить с рук всех подряд; смешивать инструменты между карантинной и общей зоной; уходить раньше без передачи смены.

Если после первого дня остались вопросы — это нормально. Спросите, когда можно прийти второй раз, какие задачи подходят новичку: выгул, социализация, фото для сайта, помощь на мероприятии. Постепенное вовлечение надёжнее, чем обещание «буду каждый день», которое не сдерживается.

Приют — не зоопарк и не фотозона. Это рабочее место, где ваше спокойствие, аккуратность и уважение к правилам напрямую влияют на то, насколько безопасно и комфортно животным ждать своего дома.""",
            False,
            volunteer_user_id,
            "volunteer",
        ),
...
После съёмки передайте файлы куратору в согласованном формате и не публикуйте в соцсетях без разрешения приюта.""",
    ...
    stale = (
        db.query(KnowledgeArticle)
        .filter(
            KnowledgeArticle.title == "Как правильно кормить кошку в период адаптации",
            KnowledgeArticle.author_user_id == volunteer_user_id,
        )
        .first()
    )
